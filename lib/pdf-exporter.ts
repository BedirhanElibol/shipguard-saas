...
<head>
  <meta charset="utf-8" />
  <title>ShipGuard Release Audit Report — ${project?.name ?? 'Target Repository'}</title>
  <style>
...
<body>
  <div class="header">
    <div>
      <div class="logo">SHIPGUARD AUDIT REPORT</div>
  ...
  <div class="grid">
    ...
    <div class="kpi-card">
      ...
      <div class="kpi-val">${23 - criticals.length - highs.length}/23 Passed</div>
    </div>
    <div class="kpi-card">
      <div style="color: #a1a1aa; font-size: 12px;">VIBEPOLISH UI MATRIX</div>
      <div class="kpi-val">${200 - (project?.uiClicheCount ?? 0)}/200 Clean</div>
    </div>
  </div>

  <div class="section">
    <h2>Audit Findings Inventory (${openFindings.length} Open Issues)</h2>
    ${openFindings.length === 0 ? '<p style="color: #10b981;">🎉 All 23 security pre-flight checks and 200 VibePolish &amp; AI Anti-Pattern rules cleared with 100/100 Readiness Score.</p>' : ''}
    ${openFindings.map(f => `
  ...
  <div class="footer">
    Generated by ShipGuard Release Gate SaaS v3.0 • Verified against 23 OWASP Security Pre-flight Checks &amp; 200 VibePolish &amp; AI Anti-Pattern Rules.
  </div>
</body>
</html>
  `;
  ...
  if (win) {
    ...
    a.href = url;
    a.download = `shipguard-audit-report-${(project?.name || 'target').toLowerCase().replace(/\s+/g, '-')}.html`;
