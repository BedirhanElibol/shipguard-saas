...
export function evaluateMobileSecurityRules(
  ...
  if (cleanContent.includes('missingCertificatePinningHazard') || (/(?:URLSession|OkHttpClient|Dio)\b/.test(cleanContent) && !/certificatePinner|pinnedCertificates|ServerTrustPolicy/i.test(cleanContent))) {
    const matchLineIdx = lines.findIndex(l => !l.trim().startsWith('//') && !l.trim().startsWith('<!--') && !l.trim().startsWith('*'));
    const lineNum = matchLineIdx !== -1 ? matchLineIdx + 1 : 1;
    findings.push({
