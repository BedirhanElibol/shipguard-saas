...
export function evaluateNoAiSlopRules(
  file: CodeFile,
  lines: string[],
  cleanContent: string,
  ...
  if (!isEligibleFile) return { findings, logs };
  ...
  // =========================================================================
  const binaryContrastPattern =
    /(?:it'?s\s+not\s+[^.!?]+[.]\s*it'?s\s+|the\s+question\s+isn'?t\s+[^.!?]+,\s*it'?s\s+|what\s+nobody\s+tells\s+you\s+is|the\s+part\s+everyone\s+misses)/i;
  ...
  if (hasSparklesImport) {
    ...
    const nonAiButtonWithSparkles =
      /<button[^>]*>[\s\S]*?<(?:Sparkles|Wand2)\b[\s\S]*?(?:Save|Submit|Export|Download|Delete|Filter|Search|Sign\s*in|Log\s*in)[\s\S]*?<\/button>/i;

    if (nonAiButtonWithSparkles.test(cleanContent)) {
      const lineIdx = lines.findIndex(
        l =>
          /<(?:Sparkles|Wand2)\b/i.test(l) ||
