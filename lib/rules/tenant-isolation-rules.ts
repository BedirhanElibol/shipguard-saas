...
export function evaluateTenantIsolationRules(
  ...
  if (cleanContent.includes('missingSearchPathTenantMigration') || (/SET\s+search_path\s*=/i.test(cleanContent) && !/\bpublic\b/i.test(cleanContent))) {
    const matchLineIdx = lines.findIndex(l => !l.trim().startsWith('//') && !l.trim().startsWith('--') && !l.trim().startsWith('*'));
    const lineNum = matchLineIdx !== -1 ? matchLineIdx + 1 : 1;
    findings.push({
  ...
  if (cleanContent.includes('unmeteredTenantWorkerJob') || (/queue\.add\s*\([^)]*(?:tenantId|orgId)/i.test(cleanContent) && !/rateLimit|concurrency/i.test(cleanContent))) {
    const matchLineIdx = lines.findIndex(l => !l.trim().startsWith('//') && !l.trim().startsWith('--') && !l.trim().startsWith('*'));
    const lineNum = matchLineIdx !== -1 ? matchLineIdx + 1 : 1;
    findings.push({
