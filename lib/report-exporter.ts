...
export function generateExecutiveMarkdown(project: Project): string {
  ...
  const verificationPayload = `${project.id || 'prj'}:${project.name}:${project.gateStatus}:${project.readinessScore}:${openFindings.length}:${openFindings.map(f => f.id + ':' + f.status).join(',')}`;
  const verificationHash = computeSha256(verificationPayload);
...
- **Digital Verification Hash:** \`SHA256: ${verificationHash}\`
...
export function generateExecutiveReportHtml(project: Project): string {
  ...
  const verificationPayload = `${project.id || 'prj'}:${project.name}:${project.gateStatus}:${project.readinessScore}:${openFindings.length}:${openFindings.map(f => f.id + ':' + f.status).join(',')}`;
  const shaSignature = computeSha256(verificationPayload);
    ...
    <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 12px 16px; margin-bottom: 20px; font-size: 8.5pt;">
      <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px;">
        ...
        <div>
          ...
          <div style="font-size: 7.5pt; color: #64748b;">Deterministic security rules applied</div>
        </div>
        <div>
