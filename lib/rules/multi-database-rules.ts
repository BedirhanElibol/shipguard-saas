...
export function detectProjectDatabases(files: CodeFile[]): DetectedStack {
  ...
  for (const file of files) {
    ...
    if (lowerPath.endsWith('package.json')) {
      if (/"(?:pg|postgres|@neondatabase\/serverless|@supabase\/supabase-js)"/i.test(content)) detectedDbs.add('PostgreSQL');
      if (/"(?:mysql|mysql2|@planetscale\/database)"/i.test(content)) detectedDbs.add('MySQL');
      if (/"(?:mongodb|mongoose)"/i.test(content)) detectedDbs.add('MongoDB');
      if (/"(?:redis|ioredis|@upstash\/redis)"/i.test(content)) detectedDbs.add('Redis');
      if (/"(?:better-sqlite3|sqlite3|@libsql\/client)"/i.test(content)) detectedDbs.add('SQLite');
      if (/"(?:@aws-sdk\/client-dynamodb|dynamoose)"/i.test(content)) detectedDbs.add('DynamoDB');
      if (/"(?:@pinecone-database\/pinecone|@qdrant\/js-client-rest|weaviate-ts-client)"/i.test(content)) detectedDbs.add('Vector DB');

      // ORMs
      if (/"@prisma\/client"|"prisma"/i.test(content)) detectedOrms.add('Prisma');
      if (/"drizzle-orm"/i.test(content)) detectedOrms.add('Drizzle');
      if (/"typeorm"/i.test(content)) detectedOrms.add('TypeORM');
      if (/"sequelize"/i.test(content)) detectedOrms.add('Sequelize');
      if (/"mongoose"/i.test(content)) detectedOrms.add('Mongoose');
    }

    // Python Manifests
    if (lowerPath.endsWith('requirements.txt') || lowerPath.endsWith('pyproject.toml')) {
      if (/psycopg2|asyncpg/i.test(content)) detectedDbs.add('PostgreSQL');
      if (/pymysql|mysqlclient/i.test(content)) detectedDbs.add('MySQL');
      if (/pymongo|motor/i.test(content)) detectedDbs.add('MongoDB');
      if (/redis/i.test(content)) detectedDbs.add('Redis');
      if (/sqlite3/i.test(content)) detectedDbs.add('SQLite');
      if (/boto3/i.test(content) && /dynamodb/i.test(content)) detectedDbs.add('DynamoDB');

      // ORMs
      if (/sqlalchemy/i.test(content)) detectedOrms.add('SQLAlchemy');
      if (/tortoise-orm/i.test(content)) detectedOrms.add('Tortoise ORM');
      if (/django/i.test(content)) detectedOrms.add('Django ORM');
    }

    // Go Manifests
    if (lowerPath.endsWith('go.mod')) {
      if (/github\.com\/jackc\/pgx|lib\/pq/i.test(content)) detectedDbs.add('PostgreSQL');
      if (/github\.com\/go-sql-driver\/mysql/i.test(content)) detectedDbs.add('MySQL');
      if (/go\.mongodb\.org\/mongo-driver/i.test(content)) detectedDbs.add('MongoDB');
      if (/github\.com\/redis\/go-redis/i.test(content)) detectedDbs.add('Redis');
      if (/mattn\/go-sqlite3/i.test(content)) detectedDbs.add('SQLite');
      if (/gorm\.io\/gorm/i.test(content)) detectedOrms.add('GORM');
    }

    // Config files
    if (lowerPath.endsWith('schema.prisma')) {
      detectedOrms.add('Prisma');
      if (/provider\s*=\s*"postgresql"/i.test(content)) detectedDbs.add('PostgreSQL');
      if (/provider\s*=\s*"mysql"/i.test(content)) detectedDbs.add('MySQL');
      if (/provider\s*=\s*"mongodb"/i.test(content)) detectedDbs.add('MongoDB');
      if (/provider\s*=\s*"sqlite"/i.test(content)) detectedDbs.add('SQLite');
    }
    ...
    if (lowerPath.includes('docker-compose')) {
      if (/image:.*postgres/i.test(content)) detectedDbs.add('PostgreSQL');
      if (/image:.*mysql|mariadb/i.test(content)) detectedDbs.add('MySQL');
      if (/image:.*mongo/i.test(content)) detectedDbs.add('MongoDB');
      if (/image:.*redis/i.test(content)) detectedDbs.add('Redis');
    }

    // Source Code Connection Patterns
    if (/(?:postgres|postgresql):\/\//i.test(content)) detectedDbs.add('PostgreSQL');
    if (/mysql:\/\//i.test(content)) detectedDbs.add('MySQL');
    if (/mongodb(?:\+srv)?:\/\//i.test(content)) detectedDbs.add('MongoDB');
    if (/redis(?:s)?:\/\//i.test(content)) detectedDbs.add('Redis');
  }
 ...
 */
export function evaluateMultiDatabaseRules(
  file: CodeFile,
  lines: string[],
  cleanContent: string,
  ...
  if (
    (lowerPath.startsWith('public/') || lowerPath.startsWith('static/') || lowerPath.startsWith('app/')) &&
    (lowerPath.endsWith('.sqlite') || lowerPath.endsWith('.sqlite3') || lowerPath.endsWith('.db'))
  ) {
    findings.push({
