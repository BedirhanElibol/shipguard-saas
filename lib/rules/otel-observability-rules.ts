...
export function evaluateOtelObservabilityRules(
  ...
  if (cleanContent.includes('serviceMissingLivenessReadinessRoutes') || (/express\(\)|FastAPI\(\)|createApp\(\)/.test(cleanContent) && !/health|liveness|readiness/i.test(cleanContent))) {
    const matchLineIdx = lines.findIndex(l => !l.trim().startsWith('//') && !l.trim().startsWith('--') && !l.trim().startsWith('#'));
    const lineNum = matchLineIdx !== -1 ? matchLineIdx + 1 : 1;
    findings.push({
  ...
  if (cleanContent.includes('uncaughtSpanErrorNotRecorded') || (/catch\s*\(\s*([a-zA-Z0-9_]+)\s*\)\s*\{/i.test(cleanContent) && /tracer\.startSpan/i.test(cleanContent) && !/span\.recordException/i.test(cleanContent))) {
    const matchLineIdx = lines.findIndex(l => !l.trim().startsWith('//') && !l.trim().startsWith('--') && !l.trim().startsWith('#'));
    const lineNum = matchLineIdx !== -1 ? matchLineIdx + 1 : 1;
    findings.push({
  ...
  if (cleanContent.includes('unboundedBatchSpanProcessorBuffer') || (/BatchSpanProcessor\(\s*[^)]*maxQueueSize:\s*(?:Infinity|\d{6,})/i.test(cleanContent))) {
    const matchLineIdx = lines.findIndex(l => !l.trim().startsWith('//') && !l.trim().startsWith('--') && !l.trim().startsWith('#'));
    const lineNum = matchLineIdx !== -1 ? matchLineIdx + 1 : 1;
    findings.push({
